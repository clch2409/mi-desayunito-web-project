...
const insumos = Array.from(insumosData.length > 0 ? insumosData : []).map(
  (insumo) => {
    return {
      ...insumo,
...
const tiposInsumo = Array.from(
  new Set(insumos.map((insumo) => insumo.tipoInsumo)),
);
const proveedores = Array.from(
  new Set(insumos.map((insumo) => insumo.proveedor)),
);
let insumosSelected = localStorage.getItem("insumosSelected")
  ? JSON.parse(localStorage.getItem("insumosSelected"))
  : [];
let insumosFiltered = insumos.sort((a, b) =>
...
const insumoButtonClearSelection = document.getElementById(
  "insumo-button-clear-selection",
);
...
insumoSearch.addEventListener("keydown", (evento) => {
  if (evento.key === "Enter") {
    evento.preventDefault();
  }
});
// insumoSearch.addEventListener("change", filtrarInsumos);
insumoSearch.addEventListener("input", filtrarInsumos);
...
function mostrarInsumosLabels() {
  insumosFiltered.forEach((insumo, index) => {
    crearLabelInputInsumo(
      `${insumo.nombre} - ${insumo.proveedor} - ${insumo.tipoInsumo}`,
      insumo.id,
    );
  });
...
function selectInsumos(evento) {
  ...
  if (targetSeleccionado) {
    eventoInsumo = evento;
    mostrarPopUp(target.id);
  } else {
    insumosSelected = insumosSelected.filter(
      (insumo) => insumo.id != target.id,
    );
...
function filtrarInsumos(evento) {
  ...
  if (tipoFiltro === tipoFilterTipoInsumo) {
    insumosFiltered = insumos.filter((insumo) =>
      insumo.tipoInsumo.toLowerCase().startsWith(nombreFiltro),
    );
  } else if (tipoFiltro === tipoFilterProveedor) {
    insumosFiltered = insumos.filter((insumo) =>
      insumo.proveedor.toLowerCase().startsWith(nombreFiltro),
    );
  } else {
    insumosFiltered = insumos.filter((insumo) =>
      insumo.nombre.toLowerCase().startsWith(nombreFiltro),
    );
  }
...
function enviarMensaje(username) {
  ...
  let newArray = [
    ...insumosSelected.sort((a, b) => a.proveedor.localeCompare(b.proveedor)),
  ];
  newArray.forEach((insumo, index) => {
    if (
      index == 0 ||
      (index >= 1 && newArray[index - 1].proveedor != newArray[index].proveedor)
    ) {
      mensaje += `\n* ${insumo.proveedor}\n\n`;
    }
